...
const ApiKey = "6604924e38e8e782599577fyta4cdd1";
...
const setLocation = async (lat, lng) => {
  ...
  const locData = await ReverseGeo.json();

  const { address } = locData;
...
class App extends React.Component {
  ...
  setLoction = (e) => this.setState({ location: e.target.value });
  ...
  componentDidMount() {
    ...
    getLatitudeAndLongitude((position) => {
      ...
      setLocation(locationData.latitude, locationData.longitude).then(
        (state) => {
          states = state.split(" ")[0];
          this.setState({ location: states });
        }
      );
    });
  }
    ...
    return (
      <div className="app">
        ...
        <Input
          ...
          onChangeLocation={this.setLoction}
        />
